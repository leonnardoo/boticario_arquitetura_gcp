...
with DAG(
    ...
    tags=["Leonnardo Pereira", "insert", "trusted", "api"],
) as dag:
    ...
    def json_to_trusted_json(**kwargs):
        ...
        for blob in blobs:
            if f"api/{nome}" in blob.name:
                ...
                json_src_file = src_file[0]['shows']

                json_list = []

                for item in range(len(json_src_file['items'])):
                    json_list.append({
                        'name': json_src_file['items'][item]['name'],
                        'description': json_src_file['items'][item]['description'],
                        'id': json_src_file['items'][item]['id'],
                        'total_episodes': json_src_file['items'][item]['total_episodes'],
